    ...
    <div className="flex min-h-screen items-center justify-center bg-[#f4f7fb] px-4 py-12 sm:py-16">
      <form onSubmit={submit} className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <label className="mt-6 block">
          ...
          <input required value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} className="ts-focus mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
        </label>
        <label className="mt-4 block">
          <span className="text-sm font-medium text-slate-700">Password</span>
          <input required type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="ts-focus mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
        </label>
